...
{
    [AssignmentExample(9,"R8C3", "193524800850376900007000000000051230001043600035960000000000500500607048006485092")] //有错误
            ...
            foreach (var item in pairCells)
            {
                ...
                foreach (var keyCells in filter2)
                {
                    ...
                    foreach (var values in findvalues)
                    {
                        ...
                        var findCell = (filterCells.Where(c => c.RestList.Contains(keyValue1) || c.RestList.Contains(keyValue2) || c.RestList.Contains(keyValue3) || c.RestList.Contains(keyValue4))).ToList();
                        if (findCell.Count==5&& HiddenValues.All(value=>filterCells.Count(cell1=>cell1.RestList.Contains(value) ) > 0))
                        {
                            ...
                            foreach (var keyCell in removeCells)
                            {
                                ...
                                if (removeValues.Any())
                                {
                                    foreach (var removeValue in removeValues)
                                    {
                                        ...
                                        "c  ",c.Location,
                                          "d  ",d.Location,
                                           "隐藏四数组"+HiddenValues.JoinString()+"位置\r\n"
                                        };
